...
## Cool!
# 2/7/23 - Re-run with cleaned bird data
# The major axes of variation appear to be along the the This confirms - you need 2 models for 


# Now make 2 dfs - one just inside, one outside

df_site_spp_inside = df_site_spp %>%
  mutate(Treatment = df_birds$Treatment) %>%
  mutate(Soil = df_birds$Soil) %>%
  filter(Treatment == "Inside")

df_site_spp_outside = df_birds %>%
  mutate(Treatment = df_birds$Treatment) %>%
  mutate(Soil = df_birds$Soil) %>%
  filter(Treatment == "Outside")

# Add up inside abundances
df_abun_inside = df_site_spp_inside %>%
    mutate(Treatment=NULL) %>%
    mutate(Soil=NULL) %>%
    mutate(X=NULL) %>%
    colSums() 

# Add up outside abundances
df_abun_outside = df_site_spp_outside %>%
  mutate(Treatment=NULL) %>%
  mutate(Soil=NULL) %>%
  mutate(X=NULL) %>%
  colSums()

# Make proportional abundances
df_p_inside = df_abun_inside / sum(df_abun_inside)
df_p_outside = df_abun_outside / sum(df_abun_outside)

# Which spp were not found outside?
# TBD

### NEW APPROACH - Indicator SPP analyses
# https://jkzorz.github.io/2019/07/02/Indicator-species-analysis.html
...
# use multipatt function to identify different spp. by red/black soil
inv_soil = multipatt(df_spp_p,
                df_birds$Soil,
                func = "r.g", control = how(nperm=9999))

summary(inv_soil, alpha=0.05)


### NOW split by soil type, and perform the same analysis
...
# Remove excess cols for analysis
df_black_multipatt = df_p_black %>%
  mutate(Treatment=NULL) %>%
  mutate(Soil=NULL)

df_red_multipatt = df_p_red %>%
  mutate(Treatment=NULL) %>%
  mutate(Soil=NULL)

# Run multipatt with treatment, seperately on black/red
inv_red = multipatt(df_red_multipatt,
                    df_p_red$Treatment,
                    func = "r.g", control = how(nperm=9999))

summary(inv_red, alpha=0.1)
# Interesting, nothing comes out here!

# Run multipatt with treatment, seperately on black/red
inv_black = multipatt(df_black_multipatt,
                    df_p_black$Treatment,
                    func = "r.g", control = how(nperm=9999))

summary(inv_black, alpha=0.1)
# species do come out here at 0.1 sig level
